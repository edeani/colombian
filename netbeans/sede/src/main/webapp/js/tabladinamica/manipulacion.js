    ...
    $(document).on('click', '.clsEliminarFila', function () {
        ...
        for (i = 0; i < tr.length; i++) {
            ...
            if (ch_.length >= 5) {
                ...
                calcularTotalFacturaByClass("contenidoFactura","totalCampo");
            }
        }
...
function calcularTotalFacturaByClass(id_contenedor, class_) {
        var valores = $("#" + id_contenedor).find("." + class_);
        var totalFactura = 0;

        for (var i = 0; i < valores.length; i++) {
            
            var valorFila = $(valores[i]).val();
            if (valorFila != '') {
                var fila = valorFila;
